...
df <- scores_df %>% select(supervised_learning_vars) %>% mutate(homeWin = as.factor(homeWin))
...
# Accuracy on holdout test data
get_test_accuracy(random_forest_fit, scoresTest)

# Do a prediction
class(predict(random_forest_fit, newdata = scoresTest %>% head(1)))
```
```{r}
# Generate input based on type
# Reference: https://stackoverflow.com/a/9950217
generate_input <- function(vtypes, index) {
  if (vtypes[[index]] == "factor") {
    textInput(paste0("randomForestPredVar_", names(vtypes)[[index]]), names(vtypes)[[index]], value = NULL)
  } else if (vtypes[[index]] == "numeric") {
    numericInput(paste0("randomForestPredVar_", names(vtypes)[[index]]), names(vtypes)[[index]], value = NULL)
  }
}

var_types <- lapply(df, class)

lapply(seq_along(var_types), generate_input, vtypes = var_types)
```
