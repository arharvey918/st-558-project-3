...
shinyServer(function(input, output, session) {
    ...
    # Data table of variable loadings
    ...
    # "suggested" variables - just some I created from other data; makes it easy to get started
    ...
    # Set the initial choices for the variables
    ...
    # Tree: 'Use suggested variables' button
    observeEvent(input$treeUseSuggestedVarsBtn, {
        updateSelectizeInput(session, "treeVars",
                             selected = suggested_vars)
    })

    # Tree training parameters, reactive on the train button click
    # Selected variables
    ...
    # Update the results section based on the selected parameters
    # Selected variables
    output$treeVarsText <- renderUI({
        tagList(lapply(treeTrainParamsVars(), span, class="badge"))
    })

    # Complexity parameter
    output$treeCpText <- renderText(treeTrainParamsCp())
    ...
    treeModel <- eventReactive(input$treeTrainButton, {
        ...
        # Add in the homeWin variable as the target variable
        ...
        tree_fit <- train(homeWin ~ ., data = scoresTrain, method = "rpart",
                                   trControl=train_control,
                                   tuneGrid = expand.grid(cp = c(input$treeCp)))

        # Return model and test accuracy
        print("Tree model building complete!")
        list(model = tree_fit,
             test_accuracy = get_test_accuracy(tree_fit, scoresTest),
             vars = names(df %>% select(-homeWin)))
    })

    # Indicator for model training progress bar
    ...
    # Update the accuracies in the results section when the tree model is built
    ...
    # Dynamically create a form for the prediction section
    output$treePredictionForm <- renderUI({
        ...
        fluidRow(
            lapply(treeModel()$vars, generate_input)
        )
    })
    ...
    # Indicator for predictions being made
    output$treePredictionMade <- renderUI({
        treePredictionInputDf()
        tags$div()
    })

    # Show the user what the inputs were for the prediction in case they change them
    ...
    # Set the choices for the variables
    ...
    # React when the train button is clicked
    ...
    # Update the results section when inputs change
    ...
    # Mtry parameter
    output$randomForestMtryText <- renderText(randomForestTrainParamsMtry())
    ...
    # Indicates when the RF model is initially created and shows a progress bar
    ...
    # Update the accuracies in the results section
    ...
    # Dynamically create the prediction form based on the selected variables
    output$randomForestPredictionForm <- renderUI({
        ...
        fluidRow(
            lapply(randomForestModel()$vars, generate_input)
        )
    })
    ...
    # Indicator for an initial prediction being made
    output$randomForestPredictionMade <- renderUI({
        randomForestPredictionInputDf()
        tags$div()
    })

    # Show the user what inputs the user set for prediction in case they change them
